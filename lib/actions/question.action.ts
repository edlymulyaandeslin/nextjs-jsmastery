...
export async function editQuestion(params: EditQuestionParams): Promise<ActionResponse<IQuestionDoc>> {
  ...
  try {
    ...
    if (tagsToRemove.length > 0) {
      ...
      await Tag.updateMany({ _id: { $in: tagIdsToRemove } }, { $inc: { question: -1 } }, { session });

      await TagQuestion.deleteMany({ tag: { $in: tagIdsToRemove }, question: questionId }, { session });

      question.tags = question.tags.filter(
        (tag: Types.ObjectId) => !tagIdsToRemove.some((id: Types.ObjectId) => id.equals(tag._id))
      );
    }
...
export async function deleteQuestion(params: DeleteQuestionParams): Promise<ActionResponse> {
  ...
  try {
    const question = await ModelQuestion.findById(questionId).session(session);
    if (!question) throw new Error("Question not found");
    ...
    // delete question
    await ModelQuestion.findByIdAndDelete(questionId).session(session);
