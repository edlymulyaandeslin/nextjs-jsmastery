  ...
  <div className="mt-16 flex w-full flex-col items-center justify-center sm:mt-36">
    ...
    {button && (
      <Link href={button.href}>
        <Button className="paragraph-medium bg-primary-500 text-light-900 hover:bg-primary-500 mt-5 min-h-[46] rounded-lg px-4 py-3">
          {button.text}
        </Button>
      </Link>
    )}
...
const DataRenderer = <T,>({ success, errors, data, empty = DEFAULT_EMPTY, render }: Props<T>) => {
  if (!success) {
    return (
      <StateSkeleton
        image={{
